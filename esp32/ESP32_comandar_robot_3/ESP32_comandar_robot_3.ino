...
#define MOTOR_B_INVERT 0      //1 para el mio
...
void setMotorsPct(int leftPct, int rightPct){
  ...
  float factorL = 0.55; // Factor para la llanta IZQUIERDA (Motor A)           0.49
  float factorR = 0.76; // Factor para la llanta DERECHA (Motor B)             0.70
  ...
  if((leftPct<10)&&(leftPct>0)){
    leftPct=15;
  }
  if((leftPct>-10)&&(leftPct<0)){
    leftPct=-15;
  }
  if((rightPct<10)&&(rightPct>0)){
    rightPct=15;
  }
  if((rightPct>-10)&&(rightPct<0)){
    rightPct=-15;

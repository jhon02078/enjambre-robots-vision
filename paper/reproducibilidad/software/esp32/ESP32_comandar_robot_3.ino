...
// Credenciales retiradas de la instantanea publica.
...
// PUERTOS UDP
// =======================
static const uint16_t CMD_PORT       = 44444;  // recibe: "M L R"
static const uint16_t DISCOVERY_PORT = 37030;  // discovery: "DISCOVER_ROBOTS"
...
const float MOTOR_A_PWM_FACTOR = 0.70;  // Canal fisico A
const float MOTOR_B_PWM_FACTOR = 0.70;  // 0.60Canal fisico B
...
void commandAck(IPAddress rip, uint16_t rport, uint32_t seq, uint32_t rxMs){
  char reply[96];
  snprintf(reply, sizeof(reply), "ACK ID=%d SEQ=%lu RXMS=%lu",
           ROBOT_ID, (unsigned long)seq, (unsigned long)rxMs);
  udpCmd.beginPacket(rip, rport);
  udpCmd.print(reply);
  udpCmd.endPacket();
}
...
void loop(){
  ...
  if (p){
    ...
    if (packetBuffer[0] == 'M'){
      int l, r;
      unsigned long seq = 0;
      int parsed = sscanf(packetBuffer, "M %d %d S %lu", &l, &r, &seq);
      if (parsed >= 2){
        uint32_t rxMs = millis();
        setMotorsPct(l, r);
        lastCmdMs = rxMs;
        if (parsed == 3){
          commandAck(udpCmd.remoteIP(), udpCmd.remotePort(), (uint32_t)seq, rxMs);
        }
      }
